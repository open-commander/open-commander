...
for ((i=max-1; i>=0; i--)); do
  COMMANDER_ENV_PYTHON_VERSION=${PYTHON[i]:-${PYTHON[0]}} \
  COMMANDER_ENV_NODE_VERSION=${NODE[i]:-${NODE[0]}} \
  COMMANDER_ENV_RUST_VERSION=${RUST[i]:-${RUST[0]}} \
  COMMANDER_ENV_GO_VERSION=${GO[i]:-${GO[0]}} \
  COMMANDER_ENV_SWIFT_VERSION=${SWIFT[i]:-${SWIFT[0]}} \
  COMMANDER_ENV_RUBY_VERSION=${RUBY[i]:-${RUBY[0]}} \
  COMMANDER_ENV_PHP_VERSION=${PHP[i]:-${PHP[0]}} \
  COMMANDER_ENV_JAVA_VERSION=${JAVA[i]:-${JAVA[0]}} \
  bash -lc '
    printf "\n\nTesting setup_universal with versions:\n"
    env | grep "^COMMANDER_ENV_" | sort
    printf "\n"
